    ...
    return (
        <div className="flex flex-col gap-2 w-full group">
            {label && (
                <label
                    htmlFor={inputId}
                    className="text-[10px] uppercase tracking-[0.4em] font-black text-white group-focus-within:text-[#C5A059] transition-colors duration-500 min-h-[32px] flex items-end"
                >
                    {label}
                </label>
            )}
            <div className="relative">
                <input
                    id={inputId}
                    className={`
            w-full px-0 py-3 bg-transparent border-b text-white placeholder:text-white/60
